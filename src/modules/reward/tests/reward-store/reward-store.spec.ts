...
test.describe('Rewards store', { tag: [REWARD_SUITE_TAGS.REWARD_STORE] }, () => {
  ...
  test(
    ...
    async ({ appManagerPage }) => {
      ...
      let selectedCountry = await rewardsStore.rewardCountry.locator('option:checked').textContent();
      expect(selectedCountry).toEqual('United States');
      ...
      selectedCountry = await rewardsStore.rewardCountry.locator('option:checked').textContent();
      expect(selectedCountry).toEqual('United States');
  ...
  test(
    ...
    async ({ appManagerPage }) => {
      ...
      const pointsBalanceLabel = await rewardsStore.pointToSpendText.textContent();
      const pendingPointsText = await rewardsStore.pendingPoints.textContent();

      expect(pointsBalanceText).toMatch(/^\d{1,3}(?:,\d{3})*$/);
      expect(pointsBalanceLabel).toBe('Points to spend');
      expect(pendingPointsText).toBe('+100 pending');
    }
  );

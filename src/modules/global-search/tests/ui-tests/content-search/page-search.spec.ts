...
test.describe(
  ...
  () => {
    ...
    test.beforeEach(async ({ appManagerApiClient }) => {
      ...
      pageName = `${faker.company.buzzAdjective()} ${faker.company.buzzNoun()}page`;
    ...
    test.afterEach(async ({ appManagerApiClient }) => {
      if (newPageID) {
        await appManagerApiClient.getContentManagementService().deleteContent(newSiteId, newPageID);
      }
      if (newSiteId) {
    ...
    test(
      ...
      async ({ appManagerHomePage }) => {
        ...
        await contentResultItem.verifyNavigationToTitleLink(newPageID,pageName,PAGE_SEARCH_TEST_DATA.content);

...
export interface FormUiFixture {
...
export interface FormUserFixture extends FormApiFixture, FormUiFixture {}

// Helper function to create API-only fixtures using existing API contexts
async function createFormApiFixture(apiContext: APIRequestContext): Promise<FormApiFixture> {
...
export const formTestFixture = base.extend<
  {
    // API-only fixtures - fast, no browser overhead
    appManagerApiFixture: FormApiFixture;
    standardUserApiFixture: FormApiFixture;

    // UI-only fixtures - browser and page components
    appManagerUiFixture: FormUiFixture;
    standardUserUiFixture: FormUiFixture;

    // Combined user fixtures - complete entry points with all helpers and services
    appManagerFixture: FormUserFixture;
    standardUserFixture: FormUserFixture;
...
>({
  ...
  appManagerApiFixture: [
    async ({ appManagerApiContext }, use) => {
      const fixture = await createFormApiFixture(appManagerApiContext);
      await use(fixture);
  ...
  standardUserApiFixture: [
    async ({ standardUserApiContext }, use) => {
      const fixture = await createFormApiFixture(standardUserApiContext);
      await use(fixture);
  ...
  appManagerUiFixture: [
    async ({ browser, appManagerApiContext }, use) => {
      const fixture = await createFormUiFixture(browser, appManagerApiContext);
      await use(fixture);
...
export const test = formTestFixture;

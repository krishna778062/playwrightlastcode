...
test.describe('Verifying invalid link msg in the editor', { tag: [TestPriority.P2] }, () => {
  test(`(invalid link msg visible in the editor)`, async ({ appManagerUINavigationHelper }) => {
  ...
  test(`(invalid link msg with random text visible in the editor)`, async ({ appManagerUINavigationHelper }) => {
    const chatAppPage = await appManagerUINavigationHelper.navigateToChatPageViaTopNavBar();
    await chatAppPage.actions.openDirectMessageWithUser(CONSTANT_DATA.USER_NAME_1);
...
test.describe('Verifying empty text msg in the editor', { tag: [TestPriority.P2] }, () => {
  test(`(Empty text box validation message visible in the editor)`, async ({ appManagerUINavigationHelper }) => {
    const chatAppPage = await appManagerUINavigationHelper.navigateToChatPageViaTopNavBar();
    await chatAppPage.actions.openDirectMessageWithUser(CONSTANT_DATA.USER_NAME_1);
...
test.describe('Verifying valid link insertion in the editor', { tag: [TestPriority.P2] }, () => {
  test(`(valid link with proper URL format inserted in the editor)`, async ({ appManagerUINavigationHelper }) => {
    const chatAppPage = await appManagerUINavigationHelper.navigateToChatPageViaTopNavBar();
    await chatAppPage.actions.openDirectMessageWithUser(CONSTANT_DATA.USER_NAME_1);

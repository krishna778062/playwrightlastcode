  ...
  () => {
    ...
    test(
      ...
      async ({ appManagerFixture, appManagerApiFixture }) => {
        ...
        await appManagerFixture.page.reload();
        await manageSitesComponentForAppManager.clickOnAddAnotherButtonAction();
        const firstMemberName = getMemBerList.result.listOfItems[0]?.name;
        if (!firstMemberName) {
          throw new Error('No members found in the site');
        }
        await addPeopleInSiteComponent.fillAddPeopleInput(firstMemberName);
        await addPeopleInSiteComponent.verifyAlreadyMemberOrFollowerTagShouldBeVisible(firstMemberName);
      }
    );

...
import { IPageCreationAssertions } from '../helpers/pageCreationAssertions';
import { AddContentModalComponent } from '../components/addContentModal';
import { IPageCreationActions, PageCreationOptions } from '../helpers/pageCreationActions';
  ...
  async verifyContentPublishedSuccessfully(title: string): Promise<void> {
    await test.step(`Verifying content was published successfully`, async () => {
      ...
      await this.verifier.verifyTheElementIsVisible(
        this.successMessage("Created page successfully - it's published"),

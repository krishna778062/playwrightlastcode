export enum ContentTestSuite {
  PAGE_APP_MANAGER = '@page-Application-Manager',
  PAGE_STANDARD_USER = '@page-Standard-User',
  ALBUM_APP_MANAGER = '@album-Application-Manager',
  ALBUM_STANDARD_USER = '@album-Standard-User',
  EVENT_APP_MANAGER = '@event-Application-Manager',
  EVENT_STANDARD_USER = '@event-Standard-User',
  FEED_APP_MANAGER = '@feed-Application-Manager',
  FEED_STANDARD_USER = '@feed-Standard-User',
  SITE_APP_MANAGER = '@site-Application-Manager',
  ATTACHMENTS = '@attachments',
  FEATURED_SITES = '@featured-sites',
  FILES_PREVIEW = '@Files-Preview',
}

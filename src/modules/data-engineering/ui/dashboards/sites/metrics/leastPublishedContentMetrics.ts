...
export enum LeastPublishedContentColumns {
  SITE_NAME = 'Site name',
  PAGE = 'Page',
  EVENT = 'Event',
  ALBUM = 'Album',
...
export class LeastPublishedContentMetrics extends TabluarMetricsComponent {
  constructor(page: Page, thoughtSpotIframe: FrameLocator) {
    super(page, thoughtSpotIframe, SITES_DASHBOARD_METRICS.LEAST_PUBLISHED_CONTENT.title);
  }

  /**
   * Verifies that the least published content data is loaded
   */
   ...
   * @param snowflakeDataArray - Array of least published content data from Snowflake
   */
  async verifyUIDataMatchesWithSnowflakeData(snowflakeDataArray: MostPublishedContentData[]): Promise<void> {
    const dataMapper = (item: MostPublishedContentData) => ({
      [LeastPublishedContentColumns.SITE_NAME]: item.site_name.trim(),
      [LeastPublishedContentColumns.PAGE]: item.pages_count.toString(),
      [LeastPublishedContentColumns.EVENT]: item.events_count.toString(),
      [LeastPublishedContentColumns.ALBUM]: item.albums_count.toString(),
      [LeastPublishedContentColumns.CONTENT_PUBLISHED]: item.total_content.toString(),
    });

    await this.compareUIDataWithDBRecords(snowflakeDataArray, dataMapper, LeastPublishedContentColumns.SITE_NAME);
  }
  ...
  async verifyCSVDataMatchesWithSnowflakeData(
    ...
    try {
      ...
      await CSVValidationUtil.validateAndAssert({
        ...
        metricName: 'Least published content',

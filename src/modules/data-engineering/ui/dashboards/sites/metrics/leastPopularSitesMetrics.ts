...
export enum LeastPopularSitesColumns {
  SITE_NAME = 'Site name',
  SITE_TYPE = 'Site Type',
...
export class LeastPopularSitesMetrics extends BaseSitesTabularMetric {
  constructor(page: Page, thoughtSpotIframe: FrameLocator) {
    super(page, thoughtSpotIframe, SITES_DASHBOARD_METRICS.LEAST_POPULAR_SITES.title);
  }

  /**
   * Verifies that the UI data matches the database data
   * @param snowflakeDataArray - Array of least popular sites data from Snowflake
   */
  async verifyUIDataMatchesWithSnowflakeData(snowflakeDataArray: MostPopularSitesData[]): Promise<void> {
    const dataMapper = (item: MostPopularSitesData) => ({
      [LeastPopularSitesColumns.SITE_NAME]: item.site_name.trim(),
      [LeastPopularSitesColumns.SITE_TYPE]: this.mapSiteTypeCodeToDisplayName(item.site_type_code),
      [LeastPopularSitesColumns.POPULARITY_SCORE]: item.total_popularity.toString(),
    });

    await this.compareUIDataWithDBRecordsHelper(snowflakeDataArray, dataMapper, LeastPopularSitesColumns.SITE_NAME);
  }

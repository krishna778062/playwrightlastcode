 ...
 *
 * This class provides:
 * - Regex-based locator that matches labels at the start of text
...
export abstract class BaseMobilePieChartMetric extends PieChartComponent {
  ...
  /**
   * Override verify method to use the regex-based locator and handle complex text extraction
  ...
  async verifySegmentLabelDataPointsAreAsExpected(params: { label: string; expectedText: string }): Promise<void> {
    ...
    // Extract only the first occurrence of the pattern to handle duplicate text
    ...
    const extractFirstOccurrence = (text: string, labelToMatch: string): string => {
      // Create a regex pattern that matches: label - number (number%)
      const escapedLabel = labelToMatch.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const pattern = new RegExp(`(${escapedLabel}\\s*-\\s*\\d+\\s*\\([^)]+%\\))`, 'i');

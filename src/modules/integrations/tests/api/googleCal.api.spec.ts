...
test.describe(
  ...
  () => {
    ...
    test(
      'verify app manager can create Google Calendar native tile via API',
      {
        tag: [TestPriority.P0, TestGroupType.SANITY, TestGroupType.SMOKE],
      },
      async ({ appManagerApiFixture }) => {
        ...
        createdTileTitle = `Google Calendar API Tile ${faker.string.alphanumeric({ length: 6 })}`;

        const payload = appManagerApiFixture.tileManagementHelper.buildValidTilePayload({
          tileInstanceName: createdTileTitle,
          connectorId: CONNECTOR_IDS.GOOGLE_CALENDAR,
          tileId: TILE_IDS.GOOGLE_CAL_DISPLAY_UPCOMING_EVENTS,
        });
    ...
    test(
      'verify app manager can retrieve Google Calendar native tile metadata via API',
      {
        tag: [TestPriority.P0, TestGroupType.SANITY, TestGroupType.SMOKE],
      },
      async ({ appManagerApiFixture }) => {
        ...
        await contentTilesApiHelper.validateTileMetadata(metadata, createdTileTitle, CONNECTOR_IDS.GOOGLE_CALENDAR);
        ApiResponseAssertions.expectSuccess(metadata);
      }
    );

    test(
      'verify app manager can delete Google Calendar native tile via API',
      {
        tag: [TestPriority.P0, TestGroupType.SANITY, TestGroupType.SMOKE],
      },
      async ({ appManagerApiFixture }) => {
    ...
    test(
      'verify app manager can get tiles by Google Calendar connector ID',
      {
      ...
      async ({ appManagerApiFixture }) => {
        ...
        await contentTilesApiHelper.validateTilesByConnector(response, TILE_IDS.GOOGLE_CAL_DISPLAY_UPCOMING_EVENTS);
    ...
    test(
      'verify app manager can create Google Calendar native tile on site dashboard via API',
      {
        tag: [TestPriority.P1, TestGroupType.REGRESSION],
      },
      async ({ appManagerApiFixture }) => {
        ...
        createdTileTitle = `Google Calendar Site Tile ${faker.string.alphanumeric({ length: 6 })}`;

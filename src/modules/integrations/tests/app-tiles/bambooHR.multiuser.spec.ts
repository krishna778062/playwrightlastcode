...
test.describe(
  'bambooHR App Tiles Multi-user Tests',
  {
    tag: [IntegrationsSuiteTags.BAMBOOHR, IntegrationsSuiteTags.ABSOLUTE, IntegrationsFeatureTags.MULTI_USER],
  },
  () => {
    ...
    multiUserTileFixture(
      'multi-user tile management for BambooHR Apply for Time Off app tile - Admin creates, EndUser verifies, Admin deletes',
      {
    ...
    multiUserTileFixture(
      'site Manager creates site with BambooHR Apply for Time Off tile, End User verifies, Site Manager deletes tile and deactivates site',
      {
        tag: [TestPriority.P1, TestGroupType.SANITY, TestGroupType.SMOKE],
      },
    ...
    multiUserTileFixture(
      'multi-user tile management for BambooHR Display Time Off Balance app tile - Admin creates, EndUser verifies, Admin deletes',
      {
        tag: [TestPriority.P1, TestGroupType.SANITY, TestGroupType.SMOKE],
      },
      async ({ adminPage, endUserPage, tileManagementHelper }) => {
        tagTest(multiUserTileFixture.info(), {
          zephyrTestId: 'INT-28181',
          storyId: 'INT-23049',
        });
    ...
    multiUserTileFixture(
      'site Manager creates site with BambooHR Display Time Off Balance tile, End User verifies, Site Manager deletes tile and deactivates site',
      {
        tag: [TestPriority.P1, TestGroupType.SANITY, TestGroupType.SMOKE],
      },
      async ({ adminPage, endUserPage, siteManagementHelper, appManagerApiClient }) => {
        tagTest(multiUserTileFixture.info(), {
          zephyrTestId: 'INT-28182',
          storyId: 'INT-23049',
        });

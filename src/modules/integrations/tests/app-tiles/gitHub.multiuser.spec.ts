...
test.describe(
  'gitHub App Tiles Multi-user Tests',
  {
    tag: [IntegrationsSuiteTags.GITHUB, IntegrationsSuiteTags.ABSOLUTE, IntegrationsFeatureTags.MULTI_USER],
  },
  () => {
    ...
    multiUserTileFixture(
      'multi-user tile management for GitHub My Open PRs app tile - Admin creates, EndUser verifies, Admin deletes',
      {
      ...
      async ({ adminPage, endUserPage, tileManagementHelper }) => {
        ...
        await adminHomeDashboard.addTile(createdTileTitle, AppName, MyOpenPRs, UI_ACTIONS.ADD_TO_HOME);
    ...
    multiUserTileFixture(
      'site Manager creates site with GitHub My Open PRs tile, End User verifies, Site Manager deletes tile and deactivates site',
      {
        tag: [TestPriority.P1, TestGroupType.SANITY, TestGroupType.SMOKE],
      },
    ...
    multiUserTileFixture(
      'multi-user tile management for GitHub Pending PR Reviews app tile - Admin creates, EndUser verifies, Admin deletes',
      {
        tag: [TestPriority.P1, TestGroupType.SANITY, TestGroupType.SMOKE],
      },
      async ({ adminPage, endUserPage, tileManagementHelper }) => {
        ...
        // Add tile, verify by both users, then remove
        const adminHomeDashboard = new HomeDashboard(adminPage, tileManagementHelper);
        await adminHomeDashboard.addTile(createdTileTitle, AppName, PendingPRs, UI_ACTIONS.ADD_TO_HOME);
        await adminHomeDashboard.verifyToastMessage(MESSAGES.ADD_TILE_SUCCESS_MESSAGE);
        await adminHomeDashboard.isTilePresent(createdTileTitle);
        const endUserHomeDashboard = new HomeDashboard(endUserPage, tileManagementHelper);
        await waitUntilTilePresentInApi(endUserPage, createdTileTitle);
    ...
    multiUserTileFixture(
      'site Manager creates site with GitHub Pending PR Reviews tile, End User verifies, Site Manager deletes tile and deactivates site',
      {
        tag: [TestPriority.P1, TestGroupType.SANITY, TestGroupType.SMOKE],
      },
      async ({ adminPage, endUserPage, siteManagementHelper, appManagerApiClient }) => {

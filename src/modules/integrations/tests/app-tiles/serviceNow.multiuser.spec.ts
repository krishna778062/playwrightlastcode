...
test.describe(
  ...
  () => {
    ...
    multiUserTileFixture(
      'verify the "Service Now App" tile is visible to end users after it has been added by the App Manager',
      {
        tag: [TestPriority.P1, TestGroupType.SANITY],
      },
      async ({ adminPage, endUserPage, tileManagementHelper }) => {
        tagTest(multiUserTileFixture.info(), {
          zephyrTestId: 'INT-29589',
          storyId: 'INT-23625',
        });
        ...
        await adminHomeDashboard.addTile(
          createdTileTitle,
          AppName,
    ...
    multiUserTileFixture(
      'verify the "Service Now App" tile is visible to end users after it has been added by the App Manager on Site Dashboard',
      {
        tag: [TestPriority.P1, TestGroupType.SANITY],
      },
      async ({ adminPage, endUserPage, siteManagementHelper, tileManagementHelper }) => {
        tagTest(multiUserTileFixture.info(), {
          zephyrTestId: 'INT-29590',
          storyId: 'INT-23625',
        });
        ...
        // Add tile, verify by both users, then remove
        await siteDashboard.addTile(createdTileTitle, AppName, DisplayRecentlyReportedTickets, UI_ACTIONS.ADD_TO_SITE);

...
initializeIntegrationConfig('workday'); //integration config is initialized for workday tenant

export default defineConfig<Options>({
  ...baseConfig,
  testDir: path.join(PROJECT_ROOT, 'src', 'modules', 'integrations', 'tests', 'gamma'),
  testMatch: '**/workday*.spec.ts',
  ...
  projects: [
    {
      ...
      use: {
        ...
        tenantConfig: getIntegrationConfig().tenant,
      },
    },
  ],
});

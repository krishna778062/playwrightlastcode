...
/**
 * Waits until all matching requests (identified by keyword) are completed with 200.
 * @param page Playwright page instance
 * @param keyword Keyword to identify requests (e.g., "X-Amz-SignedHeaders=host")
 * @param timeout Timeout in ms (default 30s)
 */
export async function waitForAllRequestsWithKeyword(
  page: Page,
  keyword: string,
  timeout: number = 30000
): Promise<void> {
  const pendingRequests: Set<Promise<Response | null>> = new Set();
  ...
  while (pendingRequests.size > 0) {
    if (Date.now() - start > timeout) {
      throw new Error(`Timeout waiting for all requests with keyword "${keyword}" to complete`);
    }

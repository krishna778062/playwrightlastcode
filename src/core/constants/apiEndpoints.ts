...
export const API_ENDPOINTS = {
  appManagement: {
    ...
    groups: {
      ...
      search: (searchTerm: string,page=1,size=20) => `/v1/chat/conversations/search?query=${searchTerm}&page=${page}&size=${size}&omit=messages&omit=users&omit=attachments`,
      listOfGroups: (page: number, perPage: number) => `/v1/chat/conversations/list?page=${page}&perPage=${perPage}&conversationType=GROUP`
    },
    identity:{

...
export class ChatService extends BaseApiClient implements IChatAdminOperations {
  ...
  async createChatGroup(
    ...
    await test.step(`Creating a chat group with name ${name}`, async () => {
      const response = await this.post(API_ENDPOINTS.appManagement.groups.create, {
        data: {
          name,
      ...
      result = await this.parseResponse<CreateGroupResponse>(response);
    });
    return result!;
  }
  ...
  async addUsersToExistingChatGroup(groupId: string, userIds: string[]): Promise<void> {
    await test.step(`Adding ${userIds.length} users to group ${groupId}`, async () => {
      await this.post(API_ENDPOINTS.appManagement.groups.addMembers, {
        data: {
          users: userIds,
  ...
  async searchChatGroupByName(groupName: string): Promise<ChatGroup> {
    const response = await this.get(API_ENDPOINTS.appManagement.groups.search(groupName));
